...
import com.saas.employee.client.AttendanceServiceClient;
...
public class ValidationUtil {
    ...
    public TenantDto getTenantById(UUID tenantId) {
        ...
        try {
            ...
            throw new ResourceNotFoundException(
                    "Tenant not found with id '" + tenantId + "' ---- " + e.getMessage());
        }
    }
    ...
    public ShiftDto getShiftById(UUID shiftId) {
        ...
        try {
            return attendanceServiceClient.getShiftById(shiftId);
        } catch (FeignException.NotFound e) {
            throw new ResourceNotFoundException(
                    "Shift not found with id '" + shiftId + "' ---- " + e.getMessage());
        }
    }

    public Employee getEmployeeById(UUID tenantId, UUID id) {
    ...
    public Employee getEmployeeByEmployeeId(UUID tenantId, String employeeId) {

        return employeeRepository
                .findByTenantIdAndEmployeeId(tenantId, employeeId)
                .filter(emp -> emp.getTenantId().equals(tenantId))
                .orElseThrow(() ->
                        new ResourceNotFoundException("Employee not found with employee id '" + employeeId + "'"));
    }

    public LocationDto getLocationById(UUID tenantId, UUID locationId) {

        try {
            return organizationServiceClient.getLocationById(locationId, tenantId);
        } catch (FeignException.NotFound e) {
            throw new ResourceNotFoundException(
                    "Location not found with id '" + locationId + "' ---- " + e.getMessage());
        }
    }

    public EducationLevelDto getEducationLevelById(UUID tenantId, UUID educationLevelId) {

        try {
            return organizationServiceClient.getEducationLevelById(educationLevelId, tenantId);
        } catch (FeignException.NotFound e) {
            throw new ResourceNotFoundException(
                    "Education level not found with id '" + educationLevelId + "' ---- " + e.getMessage());
        }
    }
    ...
    public FieldOfStudyDto getFieldOfStudyById(UUID tenantId, UUID fieldOfStudyId) {
        ...
        try {
            ...
            throw new ResourceNotFoundException(
                    "Field of study not found with id '" + fieldOfStudyId + "' ---- " + e.getMessage());
        }
    }
    ...
    public JobDto getJobById(UUID tenantId, UUID departmentId, UUID jobId) {

        try {
            List<JobDto> jobs = organizationServiceClient.getAllJobsByTenantAndDepartment(tenantId, departmentId);
            ...
            throw new ResourceNotFoundException("Job not found with id '" + jobId + "' ---- " + e.getMessage());
        }
    }

    public PayGradeDto getPayGradeById(UUID tenantId, UUID jobId, UUID payGradeId) {
        JobDto job;
        try {
        ...
        try {
            List<PayGradeDto> payGrades =
                    organizationServiceClient.getPayGradesByJobGradeId(job.getJobGradeId(), tenantId);
            ...
            throw new ResourceNotFoundException(
                    "Pay grade not found with id '" + payGradeId + "' ---- " + e.getMessage());
        }
    }

    public TitleName getTitleNameById(UUID tenantId, UUID titleNameId) {

        return titleNameRepository
                .findById(titleNameId)
    ...
    public RecruitmentDto getRecruitmentByVacancyNumber(UUID tenantId, String vacancyNumber) {
        ...
        for (RecruitmentDto recruitment : recruitments) {
            if (recruitment.getVacancyNumber().equals(vacancyNumber)) {
                ...
                throw new IllegalStateException(
                        "Recruitment with vacancy number '" + vacancyNumber + "' is found but not approved.");
            }
        }
        throw new ResourceNotFoundException("Recruitment not found with vacancy number '" + vacancyNumber
    ...
    public DutyStation getDutyStationById(UUID tenantId, UUID stationId) {

        return dutyStationRepository
                .findById(stationId)
    ...
    public Country getCountryById(UUID tenantId, UUID countryId) {

        return countryRepository
                .findByTenantIdAndId(tenantId, countryId)
                .orElseThrow(() -> new ResourceNotFoundException(
