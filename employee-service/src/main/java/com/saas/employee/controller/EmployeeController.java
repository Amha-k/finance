...
public class EmployeeController {
    ...
    public ResponseEntity<?> addEmployee(
        ...
        EmployeeResponse employee = employeeService.addEmployee(tenantId, employeeRequest, file);
    ...
    public ResponseEntity<?> getEmployeeByEmployeeId(
        ...
        EmployeeResponse employee = employeeService.getEmployeeByEmployeeId(tenantId, employeeId);
        return ResponseEntity.ok(employee);
    }
    ...
    public ResponseEntity<?> searchEmployee(
        ...
        permissionEvaluator.getEmployeeByEmployeeIdPermission(tenantId, null);

        List<EmployeeResponse> responses = employeeService.searchEmployee(tenantId, keyword);
    ...
    public ResponseEntity<?> getEmployeesByShiftId(
            @PathVariable("tenant-id") UUID tenantId,
            @PathVariable UUID shiftId) {

        permissionEvaluator.getEmployeesByShiftIdPermission(tenantId);

        List<EmployeeResponse> employees = employeeService
                .getEmployeesByShiftId(tenantId, shiftId);
        return ResponseEntity.ok(employees);
    }
    ...
    public ResponseEntity<?> getEmployeeProfileImageById(
        ...
        EmployeeResponse employee = employeeService.getEmployeeById(tenantId, employeeId);
        if (employee.getProfileImageType() == null || employee.getProfileImageType().isEmpty()) {
            return ResponseEntity.noContent().build();
        }
        MediaType mediaType = MediaType.valueOf(employee.getProfileImageType());
        byte[] profileImage = employeeService.getEmployeeProfileImageById(tenantId, employeeId);
    ...
    public ResponseEntity<?> updateEmployeeEmail(
        ...
        String response = employeeService.updateEmployeeEmail(tenantId, employeeId, email);
        return ResponseEntity.ok(response);
    }

    @PatchMapping("/update-shift/{shiftId}")
    public ResponseEntity<?> updateEmployeesShift(
            @PathVariable("tenant-id") UUID tenantId,
            @PathVariable UUID shiftId,
            @RequestBody List<UUID> employeeIds) {

        permissionEvaluator.updateEmployeesShiftPermission(tenantId);

        employeeService.updateEmployeesShift(tenantId, shiftId, employeeIds);
        return ResponseEntity.ok("Employees shift updated successfully!");
    }
